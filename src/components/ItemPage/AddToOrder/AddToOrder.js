...
export function AddToOrder(props) {
    ...
    const [orderingWithoutIngredients, setOrderingWithoutIngredients] = useState(false);
    ...
    const addItemToOrder = () => {
        if (!noIngredients) {
            ...
            const removedIngredients = missingElements(itemToAdd.ingredients, ingredients);
    ...
    return (
        <div className="add-order-wrap">
            <div className="pick-amount">
                <img className={amount > 1 ? "remove-icon" : "remove-icon grey"} src={removeIcon} onClick={() => decrement()}/>
                <p className="item-amount">{amount}</p>
                <img className="add-icon" src={addIcon} onClick={() => increment()}/>
            </div>
            <button className='add-to-order' onClick={() => addItemToOrder()}>Add to Order</button>
            { orderingWithoutIngredients ? 
                <div className="missing-ingredients">
                    <div className="missing-message-container">
                        <img className='close-icon' src={closeIcon} alt="close" onClick={() => setOrderingWithoutIngredients(false)}/>
                        <p className='missing-message'>You have removed every single ingredient from {name}, are you sure you want to order just the tableware?
                            The delivery time will be blazing fast, but it will still cost you {(price*amount).toFixed(2)} €.
                        </p>
                        <button className='add-to-order centered' onClick={() => addItemToOrder()}>Add to Order</button>
                    </div>
                </div>
            : false }
        </div>
